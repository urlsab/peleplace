...
const BookingApproved: React.FC<BookingApprovedProps> = ({
  guestName = 'אורח/ת',
  hostTitle = 'המארח',
  eventDate = '',
  hostType = 'family',
  ...
  <Html dir="rtl" lang="he">
    ...
    <Body style={main}>
      <Container style={container}>
        <Text style={heading}>הבקשה אושרה! 🎉</Text>
        <Text style={text}>
          שלום {guestName},
        </Text>
        <Text style={text}>
          שמחים לבשר שהבקשה שלך ל<strong>{hostTitle}</strong> ({hostTypeLabels[hostType] || hostType})
          {eventDate ? ` בתאריך ${eventDate}` : ''} <strong>אושרה!</strong>
        </Text>
        <Text style={text}>
          אנחנו מאחלים לך חוויה נפלאה 💛
        </Text>
        <Hr style={hr} />
