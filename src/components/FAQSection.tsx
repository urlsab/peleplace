...
const faqs = [
  ...
  },
  {
    ...
    answer:
      "ההרשמה פשוטה — בוחרים את הקטגוריה שמתאימה לכם וממלאים פרופיל מלא ישר בהתחלה (כולל ממליץ/ה).",
  },
  {
  ...
  },
  {
    ...
    answer:
      "אחרי שנרשמים בהצלחה לאתר, ניתן לצפות בלוח השבתות ושם לראות אילו שבתות פניות לאירוח ועל ידי מי. ברגע ששבת נתפסת פרטי הקשר של המארח ייחסמו.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "בהחלט! בטופס ההרשמה לאתר יש 3 אפשרויות של בחירת סוג פרופיל ואחד מהם הוא פרופיל מארח ואורח.",
  },
];
    ...
    <section id="faq" className="relative py-24 overflow-hidden">
      ...
      <div className="container relative z-10 mx-auto px-6">
        <div className="max-w-5xl mx-auto">
          ...
          >
            <h2 className="text-3xl md:text-5xl font-black leading-[1.15] font-display" style={{ color: '#FAF6F5CC' }}>
            ...
            <Accordion type="single" collapsible className="space-y-3 max-w-2xl mx-auto">
              {faqs.map((faq, i) => (
                <AccordionItem
                  key={i}
                  value={`faq-${i}`}
                  className="rounded-2xl border border-border/70 px-6 shadow-card bg-card/95 backdrop-blur-sm data-[state=open]:shadow-hover transition-shadow"
                >
                  <AccordionTrigger className="justify-center gap-3 text-center font-bold font-display text-base text-foreground py-5 hover:no-underline [&>svg]:shrink-0">
                    {faq.question}
                  </AccordionTrigger>
                  <AccordionContent className="text-center text-foreground/90 font-semibold text-sm leading-relaxed pb-5">
                    {faq.answer}
                  </AccordionContent>
                </AccordionItem>
              ))}
